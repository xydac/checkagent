...
async def test_return_policy_response(ap_cassette):
    """Agent explains return policy when asked."""
    response = await ap_cassette.arun(customer_agent, "What is your return policy?")
    assert "30 days" in response
    assert "receipt" in response


@pytest.mark.agent_test(layer="replay")
async def test_refund_timeline_response(ap_cassette):
    """Agent gives refund processing time when asked."""
    response = await ap_cassette.arun(customer_agent, "How long do refunds take?")
    assert "business days" in response


@pytest.mark.agent_test(layer="replay")
async def test_order_status_response(ap_cassette):
    """Agent looks up order status by order number."""
    response = await ap_cassette.arun(customer_agent, "What is the status of order 12345?")
    assert "12345" in response
    assert "transit" in response or "delivery" in response


@pytest.mark.agent_test(layer="replay")
async def test_rejects_prompt_injection(ap_cassette):
    """Agent refuses to comply with prompt injection attempts."""
    response = await ap_cassette.arun(
        customer_agent,
...
@pytest.mark.agent_test(layer="replay")
async def test_greeting_response(ap_cassette):
    """Agent provides a helpful greeting for general questions."""
    response = await ap_cassette.arun(customer_agent, "Hello, can you help me?")
