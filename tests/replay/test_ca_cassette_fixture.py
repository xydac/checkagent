"""Tests for the ca_cassette pytest fixture.
...
def test_cassette_fixture_record_mode(ca_cassette: CassetteFixture, tmp_path: Path) -> None:
    """ca_cassette starts in record mode when no cassette file exists."""
    assert ca_cassette.mode == "record"
    assert ca_cassette.is_recording()
    assert not ca_cassette.is_replaying()
    assert ca_cassette.recorder is not None
    assert ca_cassette.engine is None
    assert ca_cassette.cassette is None


def test_cassette_fixture_record_saves_cassette(tmp_path: Path) -> None:
    """After a test using ca_cassette in record mode, the cassette file is saved."""
