...
def render_attack_chains_markdown(simulation: dict) -> str:
    """Render an attack chain simulation result as a Markdown section.

    Suitable for appending to a compliance report generated by
    ``render_compliance_markdown()``.
    """
    lines: list[str] = []
    lines.append("\n## Attack Chain Analysis\n")

    risk = simulation.get("risk_level", "unknown").upper()
    ...
    lines.append(
        f"**Overall Risk Level:** {risk_emoji} {risk}  "
        f"**Exploitable Chains:** {chain_count}  "
        f"**Near-Misses:** {near_miss_count}\n"
    )

    chains = simulation.get("chains", [])
    if not chains:
        lines.append("*No complete attack chains identified — no co-occurring vulnerabilities.*\n")
    else:
        lines.append(
            f"> **{chain_count} attack chain{'s' if chain_count != 1 else ''} "
            f"identified.** An attacker who observes the findings below "
            f"could execute the following multi-step exploits:\n"
        )
        for chain in chains:
            impact = chain.get("impact", "unknown").upper()
            lines.append(f"### {chain['name']}  *(impact: {impact})*\n")
            lines.append(f"{chain['description']}\n")
            steps = chain.get("steps", [])
            for i, step in enumerate(steps, 1):
                evidence = step.get("evidence_probes", [])
                ev_str = ""
                if evidence:
                    probes = ", ".join(f"`{p}`" for p in evidence[:3])
                    more = f" + {len(evidence) - 3} more" if len(evidence) > 3 else ""
                    ev_str = f"  *(triggered by: {probes}{more})*"
                lines.append(f"{i}. **{step['action']}**{ev_str}")
                lines.append(f"   → {step['outcome']}")
                lines.append("")
            owasp = chain.get("owasp", [])
            if owasp:
                lines.append(f"*OWASP coverage: {', '.join(owasp)}*\n")

    near_misses = simulation.get("near_misses", [])
    if near_misses:
        lines.append("### Near-Miss Warnings\n")
        lines.append(
            "The following attack chains are **one vulnerability away** from being exploitable:\n"
        )
        for nm in near_misses:
            lines.append(f"- **{nm['chain']}** — missing: `{nm['missing_category']}`")
        lines.append("")
